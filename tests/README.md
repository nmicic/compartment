...
| **Profile trust (root)** | `root.d/profile-trust-root.sh` | compartment-root never reads `$HOME`; `/etc/compartment` ownership and mode checks; every `--profile` spelling; root audit directory; profile search **precedence** (`/etc` beats `$HOME`, and `inherit` from `/etc` cannot reach it); group-writable trust, including two users sharing a primary GID with an empty explicit member list and a paired real DAC write |
