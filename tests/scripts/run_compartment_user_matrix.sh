...
PROBE_OUT=$(LD_PRELOAD=libevil.so LD_LIBRARY_PATH=/evil LD_AUDIT=audit.so SECRET_TOKEN=s3cret \
    "${CU}" --profile "$(harness_profile test-env-deny.conf)" -- "${PROBE}" env_get LD_PRELOAD 2>/dev/null) || PROBE_RC=$?
expect_contains "LD_PRELOAD stripped" "value=(null)"

PROBE_OUT=$(LD_PRELOAD=libevil.so SECRET_TOKEN=s3cret \
    "${CU}" --profile "$(harness_profile test-env-deny.conf)" -- "${PROBE}" env_get SECRET_TOKEN 2>/dev/null) || PROBE_RC=$?
expect_contains "SECRET_TOKEN stripped" "value=(null)"

# PATH should survive (not in deny list)
PROBE_OUT=$(PATH=/usr/bin:/bin \
    "${CU}" --profile "$(harness_profile test-env-deny.conf)" -- "${PROBE}" env_get PATH 2>/dev/null) || PROBE_RC=$?
expect_contains "PATH preserved" "value=/usr/bin:/bin"

# HOME should survive
PROBE_OUT=$("${CU}" --profile "$(harness_profile test-env-deny.conf)" -- "${PROBE}" env_get HOME 2>/dev/null) || PROBE_RC=$?
...
if [ "${NO_LANDLOCK}" -eq 1 ]; then
    ...
    PROBE_OUT=$(LD_PRELOAD=evil.so \
        "${CU}" --profile "$(harness_profile test-combined.conf)" -- "${PROBE_FIX}" env_get LD_PRELOAD 2>/dev/null) || PROBE_RC=$?
...
if [ "${NO_LANDLOCK}" -eq 1 ]; then
    ...
    run_probe "$(harness_profile test-fs-readonly.conf)" fd_list
    if [ -n "${PROBE_OUT}" ]; then
