...
# $OWNER is the root-owned copy of the loader inside $TMP, and with
# --self-protect armed it is the ONLY image that can unpin. Deleting $TMP after
# a failed unpin would leave a self-protected policy on the box that nothing
# can remove short of a reboot, so the rm is conditional on the unpin working.
cleanup() {
	[ -n "${DPID:-}" ] && kill "$DPID" 2>/dev/null
	_ok=1
	COMPARTMENT_BPF_PASSPHRASE="$PASS" "${OWNER:-$BIN}" --unpin >/dev/null 2>&1 || _ok=0
	for _ in $(seq 1 60); do
		[ "$(bpftool prog show 2>/dev/null | grep -c 'name comp_')" -eq 0 ] && break
		sleep 1
	done
	[ "$(bpftool prog show 2>/dev/null | grep -c 'name comp_')" -eq 0 ] || _ok=0
	if [ "$_ok" -eq 0 ] && [ -n "${OWNER:-}" ] && [ -e "${OWNER:-}" ]; then
		echo "[bpf-bench] --unpin did not complete; KEEPING $OWNER — it is the" >&2
		echo "            only image authorised to unpin this policy. Run:" >&2
		echo "            COMPARTMENT_BPF_PASSPHRASE='$PASS' $OWNER --unpin" >&2
		return 0
	fi
	rm -rf "$TMP"
}
...
# A leg that produced no samples, or samples from a run where every syscall
# errored, must not silently become "0.0" and then a percentage. awk with no
# input prints 0.0 quite happily, and the deltas downstream would be fiction.
run_leg() {
	_raw="$TMP/leg.$$"
	: > "$_raw"
	for _ in $(seq 1 "$REPS"); do
		"$TMP/bench" "$ITERS" >> "$_raw" || return 1
	done
	_n=$(grep -c '^BPFBENCH ' "$_raw")
	[ "$_n" -eq "$REPS" ] || { echo "[bpf-bench] leg produced $_n/$REPS samples" >&2; return 1; }
	_err=$(sed -n 's/.*errors=\([0-9]*\).*/\1/p' "$_raw" | awk '{s+=$1} END{print s+0}')
	[ "$_err" -eq 0 ] || { echo "[bpf-bench] leg had $_err failed bpf() calls; the timing is of the deny path, not the allow path" >&2; return 1; }
	sed -n 's/.*ns_per_call=\([0-9.]*\).*/\1/p' "$_raw" \
		| sort -n | awk '{v[NR]=$1} END{if (NR) printf "%.1f\n", v[int((NR+1)/2)]}'
}
...
base=$(run_leg) || { echo "[bpf-bench] SKIP (baseline leg failed)"; exit 77; }
...
off=$(run_leg) || { echo "[bpf-bench] SKIP (flag-off leg failed)"; exit 77; }
...
on=$(run_leg) || { echo "[bpf-bench] SKIP (flag-on leg failed)"; exit 77; }
...
# The whole premise of the flag-off leg is that comp_bpf_map is NOT loaded.
# If the two legs pinned the same number of links, the "off" number is not
# measuring what this bench says it measures.
[ "$on_links" -eq $((off_links + 1)) ] || {
	echo "[bpf-bench] FAIL: armed pin has $on_links links, unarmed has $off_links; expected exactly one more (comp_bpf_map)" >&2
	exit 1
}
