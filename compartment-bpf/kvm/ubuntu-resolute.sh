...
# usage: ubuntu-resolute.sh [--check]
#
...
packages:
  ...
  # release, for four packages. postgresql (not just -common) is needed
  # because postgres.sh asserts against a live, online cluster; chrony
  # because pin-regression T4.4 reloads a real chronyd profile and the
  # Noble cloud image uses systemd-timesyncd, so /etc/chrony/chrony.conf
  # is absent there and one of five pin/unpin regression tests skipped.
  - aide
  - postgresql-common
  - postgresql
  - chrony
...
if osinfo-query os 2>/dev/null | awk '{print $1}' | grep -qx ubuntu26.04; then
  ...
  echo "NOTE: this host's osinfo-db does not know ubuntu26.04 (osinfo-db 0.20250606"
  echo "      still does not), so the guest is DESCRIBED to libvirt as ${OS_VARIANT}."
  echo "      Harmless — virtio everywhere, and the variant only feeds device/feature"
  echo "      hints — but it is what \`virsh dominfo\` will report."
fi
