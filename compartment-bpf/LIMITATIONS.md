...
address on its own. This document is the operator-facing summary of
...
| **32-bit (compat) ioctl callers** (v0.8) | `file_ioctl` gates `FS_IOC_SETFLAGS` / `FS_IOC32_SETFLAGS` / `FS_IOC_FSSETXATTR` / `FS_IOC_SETVERSION` (`chattr +i/+a`, project ids) on `no-chmod` seals; these mutate inode metadata through `->fileattr_set` with no `inode_setattr` or xattr hook, so through v0.7 nothing saw them. Since kernel 6.8 the compat `ioctl(2)` entry dispatches through `security_file_ioctl_compat()`, which is not hooked, so a 32-bit process on a 64-bit kernel can still `chattr` a sealed inode. On 6.6/6.7 compat callers hit `file_ioctl` with the `FS_IOC32_*` encodings, which are covered. Kernels ≥ 6.17 additionally offer `inode_file_setattr`, which sees both entry points. Witness: `tests/bypass/18-chattr-no-chmod.sh` (native path). | Build with `CONFIG_IA32_EMULATION=n` / `CONFIG_COMPAT=n`, or seccomp-deny the compat arch (`AUDIT_ARCH_I386`) for privileged services. v0.9 candidate: optional `file_ioctl_compat` / `inode_file_setattr` programs autoloaded only when the running kernel's BTF has the hook. |
| **Strict-launch marker vs. failed exec** (closed v0.8) | Through v0.7 the marker was written in `bprm_check_security`, which fires from `search_binary_handler()` before the exec point of no return. An exec of the sealed launcher that failed afterwards — `dup_fd` ENOMEM under a memcg limit with a `CLONE_FILES` sibling alive, `exec_mmap`, `de_thread` — returned `-errno` to the caller's OLD image carrying a valid marker whose target matched its exe. A task already running the actor target under `LD_PRELOAD` (exe == target, no marker) could therefore acquire a marker without ever running the launcher and satisfy every strict-launch condition. v0.8 moves marker set/keep/clear to `bprm_committed_creds`, which runs only once the new image is installed; an unresolvable exec target drops any existing marker. No residual is known. Listed because `experimental/EXEC-DOMAIN-SPEC.md` §6 and the strict-launch feasibility SPEC still describe the hook as `bprm_check_security`. | Upgrade to v0.8: `--unpin` (the loader still sweeps the legacy `comp_bprm_check_security` pin) then `--pin`. Any exec-*deny* policy added later (exec ACL) still belongs at `bprm_check_security`; only marker *mutation* moved. |
